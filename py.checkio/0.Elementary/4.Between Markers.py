...
beginning of a string.If there is no final marker then the ending should be 
considered as the ending of a string.If the initial and final markers are 
missing then simply return the whole string.If the final marker is standing in 
front of the initial one then return an empty string.
...
def between_markers(text: str, begin: str, end: str) -> str:
    ...
    if begin >= 0 and end >= 0:
        if begin > end:
            return ""
        return text[begin + beginLen:end]
    if begin >= 0 or end >= 0:
        if begin >= 0:
            return text[begin + beginLen:]
        return text[:end]
    return text
