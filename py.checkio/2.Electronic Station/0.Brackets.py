...
bracket is open, then it must be closed with a closing bracket of the same type. 
The scope of a bracket must not intersected by another bracket. In this task you 
should make a decision, whether to correct an expression or not based on the 
...
def checkio(expression):
    ...
    for i in expression:
        if i in bracket_open: 
            ...
            if len(count) == 0: 
                return False
            elif bracket_open.find(count[-1]) == bracket_close.find(i): 
                count.pop()
            else: 
                break
